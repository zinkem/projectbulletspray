...
public class Level {
	...
	public static enum Layer {
		BACKGROUND, TRIGGERS, STATIC, FX, HOSTILE, ENEMY, FRIENDLY, PLAYER, FOREGROUND, HUD
	}
	...
	public void setupCollisionHandlers() {
		...
		collisionHandlers.add(new QuadLayerCollisionHandler(
				getLayer(Layer.PLAYER), getLayer(Layer.STATIC)) {
			...
			public void collide(Entity body1, Entity body2) {
				if (body1.isActive() && body2.isActive()) {
					...
					// Kill the player, do any sort of updates to the game
					// necessary
					body1.kill();
		...
		collisionHandlers.add(new QuadLayerCollisionHandler(
				getLayer(Layer.PLAYER), getLayer(Layer.ENEMY)) {
			...
			public void collide(Entity body1, Entity body2) {
				if (body1.isActive() && body2.isActive()) {
					...
					e.setPosition(body2.getCenterPosition().translate(
							new Vector2D(ry, rx)));
					e.setCustomAnimation(new AnimateOnce(75));
					add(e, Layer.FX);
